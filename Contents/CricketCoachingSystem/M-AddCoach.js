...
//     if (!name || !age || !experience || !contact || !username || !password) {
...
//     try {
//       const response = await fetch(ip_adress + '/api/admin/add_coach', {
//         method: 'POST',
//         headers: {
...
//         // Handle duplicate coach case
//         Alert.alert('Error', 'Coach already exists in the database');
//       } else {
//         Alert.alert('Error', data.error || 'Something went wrong');
//       }
//     } catch (error) {
...
//             <Text style={styles.popupText}>Coach added successfully.</Text>
//             <TouchableOpacity
...
const AddCoachScreen = ({navigation}) => {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await fetch(`${ip_adress}/manager/add_coach`, {
        method: 'POST',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify(coachData),
      });
      ...
      if (response.status === 201) {
        ...
        Alert.alert('Error', data.message || 'Failed to add coach');
      }
    } catch (error) {
          ...
          <View style={styles.modalContainer}>
            <View style={styles.modalContent}>
              ...
              <Text style={styles.modalText}>
                Coach has been added successfully.
              </Text>
              <TouchableOpacity
...
// } from 'react-native';
// import CustomGradient from './components/CustomGradient';
// import {Dialog, Button} from 'react-native-paper'; // Import Dialog and Button from react-native-paper
...
//   const [password, setPassword] = useState('');
//   const [isLoading, setIsLoading] = useState(false);
//   const [isSuccessPopupVisible, setIsSuccessPopupVisible] = useState(false); // State for popup visibility
...
//   // Close the success popup and navigate back
//   const handlePopupClose = () => {
//     setIsSuccessPopupVisible(false);
//     navigation.goBack(); // Navigate back after closing the popup
//   };
